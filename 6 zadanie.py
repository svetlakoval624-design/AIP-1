...
def is_lucky_ticket(ticket):
    mid=len(ticket) // 2
    first_sum=sum(int(ticket[i]) for i in range(mid))
    second_sum=sum(int(ticket[i]) for i in range(mid, len(ticket)))
    return first_sum == second_sum
print(is_lucky_ticket("385916"))
print(is_lucky_ticket("231002"))
print(is_lucky_ticket("33"))
print(is_lucky_ticket("2341"))
